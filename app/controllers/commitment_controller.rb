class CommitmentController < ApplicationController
	...
	def ViewCommitments
	    ...
	    begin
			...
			commitments = Commitment.where(helper_id: ind.id, commitmentStatus_id: offerAcceptedCS.id).or(Commitment.where(helper_id: ind.id, commitmentStatus_id: completionRequestCS.id)).or(Commitment.where(helper_id: ind.id, commitmentStatus_id: completionRejectedCS.id))
